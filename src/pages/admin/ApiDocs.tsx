...
const apiSections: ApiSection[] = [
  ...
  },
  {
    ...
    description: "Envio de mensagens via WhatsApp (Evolution API). Migração para OpenClaw planejada.",
    endpoints: [
      {
        method: "POST",
        path: "supabase.functions.invoke('send-whatsapp')",
        summary: "Enviar mensagem WhatsApp (Edge Function)",
        description: "Envia mensagem de texto para um ou mais destinatários via Evolution API. Suporta templates com variáveis. Requer role admin.",
        auth: "admin",
        params: [
          { name: "recipients", type: "array", required: true, description: "[{ phone, name?, student_id? }]" },
          { name: "message_body", type: "string", required: false, description: "Texto da mensagem (direto)" },
          { name: "template_id", type: "uuid", required: false, description: "ID do template" },
          { name: "variables", type: "object", required: false, description: "Variáveis do template { nome, turma, ... }" },
        ],
        response: `{ sent: number, failed: number, results: [{ phone, success, messageId?, error? }] }`,
      },
      {
        method: "GET",
        path: "supabase.from('whatsapp_templates').select('*')",
        summary: "Listar templates de mensagem",
        description: "Retorna templates de mensagem configurados com variáveis dinâmicas.",
        auth: "admin",
      ...
      },
      {
        ...
        description: "Retorna log de todas as mensagens enviadas com status (sent/failed) e detalhes de erro.",
        auth: "admin",
      },
    ],
  },
